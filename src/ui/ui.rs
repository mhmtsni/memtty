...
impl MyApp {
    ...
    //             self.scroll_offset = (self.scroll_offset - delta).max(0).min(max_offset);
    ...
    //             Key::Named(Named::ArrowRight) if modifiers.alt() => b"\x1b[1;5C".to_vec(),
    //             Key::Named(Named::ArrowLeft) if modifiers.alt() => b"\x1b[1;5D".to_vec(),
